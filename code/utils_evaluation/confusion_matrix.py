...
def step1_score_format(language_option):
    data_path = "./scripts/result/pipeline/step1-10%/single"
    all_data_name = os.listdir(data_path)
    step1_score = {language_option : {}}

    for data_name in [all_data for all_data in all_data_name if all_data in ['deepseek', 'llama', 'ministral', 'qwen']]:
    ...
    for idx in range(490):    
        ...
        for vote_key in total_voting_data[str(idx)].keys():
            ...
            try:
                ...
                # if logit is 0 but pipeline predicted, set logit to 1
...
logging.basicConfig(
    level=logging.INFO,  # set log level (INFO and above only)
    format="%(message)s",  # set log format
    handlers=[
        logging.FileHandler(log_filepath, mode="w",encoding="utf-8"),  # save to file (no terminal)
    ]
)
...
# calculate accuracy and f1 score (f1 is calculated with average="macro" for multi-class)       
